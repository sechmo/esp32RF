...
const uint8_t maxPayloadLen = 67;
...
const uint8_t maxMsgLen = (maxPayloadLen - headerLen - 3);
...
const uint8_t  rampInc = (rxRampLen/rxSamples);
const uint8_t  rampTransition = rxRampLen/2;
const uint8_t  rampAdjust = 9;
const uint8_t  rampIncRetard = (rampInc-rampAdjust);
const uint8_t  rampIncAdvance = (rampInc+rampAdjust);
...
protected:
    ...
    uint8_t _txBuf[(maxPayloadLen * 2) + preambleLen];
